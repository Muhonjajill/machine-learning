...
logger.info(daily.head(10))
logger.info(daily.columns)

daily = daily.replace([np.inf, -np.inf], np.nan).fillna(0)

daily['anomaly_z'] = daily['zscore'].abs() > 3

logger.info(f"Z-score anomalies: {daily['anomaly_z'].sum()}")
...
logger.info(f"Model Stability Score: {stability_score:.3f}")
...
logger.info(f"Out-of-Sample Score Separation: {separation:.4f}")
...
logger.info(cust_conf[['custid', 'custname', 'confidence']].head(10))
...
logger.info(f"Detected {len(payload)} anomalies via IsolationForest")
...
def send_anomalies(payload, backend_url = os.getenv("BACKEND_URL") + "/api/anomalies"):
    try:
        ...
        logger.info(f"Sent anomalies to backend: {resp.status_code}")
    except Exception as e:
        logger.error(f"Failed to send anomalies: {e}")
