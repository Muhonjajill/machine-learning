...
logging.basicConfig(
    filename="cashin_forecast.log",
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s"
)

logging.info("Cash-in forecast script started")
...
logger.info(f"Retrieved data for {len(terminals)} terminals (joined with terminals info)")
...
for terminal in terminals:
    ...
    if len(terminal_data) < 3:
        logger.warning(f"Skipping {terminal} (not enough data)")
        continue
    ...
    logger.info(f"Confidence for {terminal}: {terminal_conf:.2f}")
...
if all_forecasts:
    ...
    logger.info(f"Saved combined forecast to: {csv_path}")
else:
    logger.warning("No forecasts generated.")

logger.info("Fetching REAL actual cash-in data...")
...
logger.info("Merged forecast + actual data with anomaly scoring")
...
logger.info("Data cleaned for JSON serialization (guaranteed)")
...
try:
    ...
    logger.info(f"Sending forecast data to backend: {backend_url}")
    ...
    if response.status_code == 200:
        logger.info("Forecast data sent successfully to backend!")
    else:
